...
- **Face UI (ui@1)** — declarative UI composition DSL. `FaceUiDoc` with `$ref` (data binding) and `$action` (event binding). `materializeFaceUiDoc()` with deterministic `nodeId` generation. `FaceUi` React component. `createFaceUiRegistry()` for type → component resolution. Zod schema validation.
...
- **Codegen** — `generateCode()`: materialize ui@1 documents into React JSX, Vue SFC, or HTML.
- **Recipe Packs** — 5 B2B ui@1 templates: Dashboard, CRUD Table, Form, Settings, List-Detail. Bundled in `packs/recipes/`.
