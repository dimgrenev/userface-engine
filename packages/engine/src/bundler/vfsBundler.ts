...
export interface BundleResult {
  ...
  filesUsed: string[];
  error?: string;
  /** Soft errors: unresolved imports that were skipped instead of crashing the bundle. */
  moduleErrors?: Array<{ module: string; from: string; message: string }>;
}
...
function hasTypeScriptSyntax(src: string): boolean {
  try {
    const s = String(src || '');
...
function transpileBundleWithBabel(code: string, babelInstance?: any): { success: boolean; code: string; error?: string } {
  try {
    ...
    if (!out.trim()) return { success: false, code, error: 'Babel transform produced empty output' };
    return { success: true, code: out };
  } catch (e: any) {
    return { success: false, code, error: e?.message || String(e) };
  }
}
...
function parseExportSpecifiers(list: string): Array<{ exported: string; local: string }> {
  try {
...
function stripModuleSyntax(src: string, opts?: { isEntry?: boolean }): string {
  ...
  out = out.replace(/^\s*export\s+(const|let|var|function|class)\s+([A-Za-z0-9_$]+)/gm, (m, _kind, name) => {
    if (name) exportedNames.add(String(name));
...
function isIgnorableAssetImport(rel: string): boolean {
...
const babelTranspileCache = new Map<string, { success: boolean; code: string; error?: string }>();

function pruneMap<V>(map: Map<string, V>, max: number): void {
...
function parseCssModuleClassMap(cssText: string): Record<string, string> {
  try {
...
function toSafeJsString(s: string): string {
  return String(s || '').replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$\{/g, '\\${');
}

function parseValueNamedSpecifiers(list: string): string[] {
...
function resolveNonRelativeKey(rel: string, vfs: Record<string, VfsFileLike>): string | null {
  try {
    ...
    for (const c of candidates) {
      if (vfs[c] && typeof vfs[c].content === 'string') return c;
    }
    const suffixes = candidates.map((c) => c.replace(/^\/+/, ''));
    const keys = Object.keys(vfs || {});
    for (const key of keys) {
      if (!isCodeFile(key)) continue;
      for (const suffix of suffixes) {
        if (key === suffix || key.endsWith(`/${suffix}`)) return key;
      }
    }
    return null;
  } catch {
...
function transformImportsToRequires(params: {
  key: string;
  src: string;
  resolveKey: (fromKey: string, rel: string) => string | null;
  vfs: Record<string, VfsFileLike>;
}): string {
  ...
  out = out.replace(/^([ \t]*)import\s+([A-Za-z0-9_$]+)\s+from\s+['"]([^'"]+\.module\.css)['"][ \t]*;?/gm, (_m, lead, ident, rel) => {
    try {
      ...
      const css = vfs[cssKey]?.content ? String(vfs[cssKey].content) : '';
  ...
  out = out.replace(/^([ \t]*)import\s+['"]([^'"]+)['"][ \t]*;?/gm, (_m, lead, rel) => {
    try {
      ...
      if (isIgnorableAssetImport(r)) return lead;
  ...
  out = out.replace(/^([ \t]*)import\s*\{([^}]*)\}\s+from\s+['"]([^'"]+)['"][ \t]*;?/gm, (_m, lead, named, rel) => {
    ...
    if (!dep) return lead;
    const destructItems = parseValueNamedSpecifiers(String(named || ''));
    if (destructItems.length === 0) return lead;
...
function transformExportsToCjs(src: string, _opts?: { isEntry?: boolean }): { code: string } {
  let out = String(src || '');
  const exportedNames = new Set<string>();
...
export function bundleFromVfs(entryKey: string, vfs: Record<string, VfsFileLike>, options: BundlerOptions = {}): BundleResult {
  const moduleErrors: Array<{ module: string; from: string; message: string }> = [];
  try {
    const visited = new Set<string>();
    const order: string[] = [];

    const resolveKey = (fromKey: string, rel: string): string | null => {
      ...
      if (!isCodeFile(raw) && isIgnorableAssetImport(raw)) return null;
      const candidates = [
        `${raw}.tsx`,
        `${raw}.ts`,
        `${raw}.jsx`,
      ...
      if (isCodeFile(raw)) candidates.unshift(raw);
      for (const c of candidates) { if (vfs[c] && typeof vfs[c].content === 'string') return c; }
      return null;
    };
    ...
    const dfs = (key: string) => {
      ...
      if (!file) {
        ...
        if (key === entryKey) throw new Error(`File not found in VFS: ${key}`);
      ...
      for (const rel of deps) {
        ...
        if (!depKey) {
          if (!isRel) continue;
          if (isIgnorableAssetImport(r)) continue;
    ...
    return { success: false, code: '', filesUsed: [], moduleErrors, error: e?.message || String(e) };
  }
}

export function needsBundling(source: string): boolean {
  try {
