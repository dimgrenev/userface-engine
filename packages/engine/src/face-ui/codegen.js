...
function generateReactProps(props) {
    if (!props)
        return '';
...
function generateReactNode(node, depth) {
    ...
    const childrenStr = node.children.map(child => {
        ...
        if (typeof child === 'number' || typeof child === 'boolean')
            return `${indent}  {${child}}`;
        if (child && typeof child === 'object' && 'type' in child)
            return generateReactNode(child, depth + 1);
        return '';
    }).filter(Boolean).join('\n');
    return `${indent}<${node.type}${propsStr}>\n${childrenStr}\n${indent}</${node.type}>`;
}
export function generateReactCode(doc, options = {}) {
    const root = doc.root;
    const compName = options.componentName || 'GeneratedComponent';
    const usedComponents = new Set();
    collectComponents(root, usedComponents);
    const imports = Array.from(usedComponents).sort().map(c => `import { ${c} } from '@/components/${c}';`).join('\n');
...
function generateVueProps(props) {
    if (!props)
        return '';
    const entries = Object.entries(props).map(([k, v]) => {
        if (isActionRef(v)) {
...
function generateVueNode(node, depth) {
    const indent = '  '.repeat(depth);
    ...
    const childrenStr = node.children.map(child => {
        ...
        if (typeof child === 'number' || typeof child === 'boolean')
            return `${indent}  {{ ${child} }}`;
        if (child && typeof child === 'object' && 'type' in child)
            return generateVueNode(child, depth + 1);
        return '';
    }).filter(Boolean).join('\n');
    return `${indent}<${node.type}${propsStr}>\n${childrenStr}\n${indent}</${node.type}>`;
}
export function generateVueCode(doc, options = {}) {
    const root = doc.root;
    const usedComponents = new Set();
    collectComponents(root, usedComponents);
    const imports = Array.from(usedComponents).sort().map(c => `import { ${c} } from '@/components/${c}.vue';`).join('\n');
...
// HTML Generator
// ============================================================================
function generateHtmlProps(props) {
    if (!props)
        return '';
    const entries = Object.entries(props).map(([k, v]) => {
        if (isActionRef(v)) {
...
function generateHtmlNode(node, depth) {
    const indent = '  '.repeat(depth);
    ...
    const childrenStr = node.children.map(child => {
        ...
        if (typeof child === 'number' || typeof child === 'boolean')
            return `${indent}  ${child}`;
        if (child && typeof child === 'object' && 'type' in child)
            return generateHtmlNode(child, depth + 1);
        return '';
    }).filter(Boolean).join('\n');
    return `${indent}<${tagName}${fullProps}>\n${childrenStr}\n${indent}</${tagName}>`;
}
export function generateHtmlCode(doc, options = {}) {
    const root = doc.root;
