...
import { loadRegistryManifest } from '../registryManifest.js';
/**
...
function resolvePatternsDir() {
    ...
    for (const dir of candidates) {
        try {
            ...
            if (files.some(f => f.endsWith('.pattern.json')))
                return dir;
        }
        catch { }
    }
...
function loadBuiltinPatterns() {
    try {
        ...
        return files.map(f => {
            const raw = JSON.parse(readFileSync(resolve(patternsDir, f), 'utf-8'));
            return {
                id: `pattern/${raw.id}`,
                name: raw.name,
...
let _builtinPatterns = null;
let _faceUiComponentTypes = null;
...
function getBuiltinPatterns() {
    if (!_builtinPatterns)
        _builtinPatterns = loadBuiltinPatterns();
 ...
 */
function loadCustomPatterns(filePaths) {
    const rules = [];
 ...
 */
export function loadPatternById(patternId) {
    try {
        const patternsDir = resolvePatternsDir();
        if (!patternsDir)
            return null;
...
function collectAllNodes(root) {
    const result = [];
    function walk(node, path, depth, parentTypes) {
        result.push({ node, path, depth, parentTypes });
        if (!Array.isArray(node.children))
            return;
        let childIdx = 0;
        for (const ch of node.children) {
            if (ch == null || typeof ch !== 'object' || !('type' in ch))
                continue;
            walk(ch, [...path, childIdx], depth + 1, [...parentTypes, node.type]);
            childIdx++;
        }
    }
    walk(root, [0], 1, []);
...
function isUppercaseComponentType(type) {
    return /^[A-Z]/.test(type);
}
function resolveFaceUiRegistryPath() {
    const candidates = [
...
function getFaceUiComponentTypes() {
    if (_faceUiComponentTypes)
        return _faceUiComponentTypes;
    ...
    for (const type of FALLBACK_FACE_UI_COMPONENT_TYPES)
        componentTypes.add(type);
...
// Structural checks
// ---------------------------------------------------------------------------
function checkStructural(doc, maxDepth) {
    const violations = [];
    ...
    for (const { node, path, depth, parentTypes } of nodes) {
        ...
        if (Array.isArray(node.children) && node.children.length > 1) {
            const nodeChildren = node.children.filter((ch) => ch != null && typeof ch === 'object' && 'type' in ch);
    ...
    for (const { node } of nodes) {
        if (!Array.isArray(node.children))
            continue;
        const childNodes = node.children.filter((ch) => ch != null && typeof ch === 'object' && 'type' in ch);
        const accentButtons = childNodes.filter(ch => ch.type === 'Button' && ch.props?.variant === 'accent');
...
// ---------------------------------------------------------------------------
function checkContracts(doc, registry) {
    const violations = [];
    const nodes = collectAllNodes(doc.root);
    ...
    for (const { node } of nodes) {
        ...
        for (const propName of Object.keys(nodeProps)) {
            if (propName === 'key' || propName === 'className' || propName === 'style' || propName === 'id')
                continue;
...
// ---------------------------------------------------------------------------
function registryBoundaryConfigViolation(ruleId, description) {
    return {
        ruleId,
        description,
        severity: 'error',
...
function checkRegistryBoundary(doc, registryManifestPath) {
    if (!registryManifestPath) {
        return [registryBoundaryConfigViolation('composition/registry-boundary-missing-manifest', 'Registry boundary enforcement requires registryManifestPath')];
    }
    let manifest;
    try {
        ...
        return [registryBoundaryConfigViolation('composition/registry-boundary-invalid-manifest', `Registry boundary manifest could not be loaded: ${detail}`)];
    }
    ...
    const publicManifestTypes = new Set(manifest.components
        .filter(component => component.registryVisibility === 'public')
        .map(component => component.name));
    const outsidePublicManifestTypes = new Set();
    for (const { node } of collectAllNodes(doc.root)) {
        if (!isUppercaseComponentType(node.type))
            continue;
        if (faceUiTypes.has(node.type) || publicManifestTypes.has(node.type))
            continue;
        outsidePublicManifestTypes.add(node.type);
    }
    return [...outsidePublicManifestTypes].map(type => {
...
// ---------------------------------------------------------------------------
function checkRefs(doc, context, actionNames) {
    const violations = [];
    const nodes = collectAllNodes(doc.root);
...
// Pattern compliance
// ---------------------------------------------------------------------------
function checkPatterns(doc, patternNames, customPatternFiles, enforceComponentSelection = false) {
    const violations = [];
    const nodes = collectAllNodes(doc.root);
    ...
    for (const patternName of patternNames) {
            ...
            for (const req of pattern.requires) {
                ...
                if (req.props && count > 0) {
                    ...
                    const withRequiredProps = matchingNodes.filter(n => {
                        ...
                        return Object.entries(req.props).every(([k, v]) => nodeProps[k] === v);
                    });
        ...
        if (enforceComponentSelection && pattern.componentSelection) {
            ...
            for (const { node } of nodes) {
                if (!isUppercaseComponentType(node.type) || allowedTypes.has(node.type))
                    continue;
...
// ---------------------------------------------------------------------------
function computeScores(violations) {
    const byCat = { structural: 0, contract: 0, a11y: 0, complexity: 0, pattern: 0 };
    const weights = { error: 10, warning: 3, info: 1 };
    for (const v of violations) {
        const cat = v.category;
        if (cat in byCat)
            byCat[cat] += weights[v.severity] * v.confidence;
    }
    const maxPenalty = 100;
    const score = (penalty) => Math.max(0, Math.round(maxPenalty - penalty));
...
function applyBudget(violations, budget) {
    ...
    switch (budget) {
        case 'llm': return sorted.filter(v => v.severity === 'error').slice(0, 5);
...
// Public API
// ---------------------------------------------------------------------------
export function validateComposition(doc, options = {}) {
    const start = performance.now();
    const maxDepth = options.maxDepth ?? 12;
    const budget = options.budget || 'verbose';
    const allViolations = [];
    ...
    if (options.patterns?.length) {
        allViolations.push(...checkPatterns(doc, options.patterns, options.customPatternFiles, options.enforceComponentSelection));
    }
