...
3. For new screens/pages, compose a face schema v1 JSON document that describes the interface structure
...
- Face schema v1 is a declarative IR for UI composition — it's framework-agnostic
...
- `composition_validate({ doc, registryDir, patterns })` — Validate face schema v1 structure
- `ui_materialize({ doc, framework })` — Generate code from face schema v1
